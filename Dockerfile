FROM php:7.1.16-zts-jessie

LABEL com.example.vendor="ehime" \
      version="1.0"

MAINTAINER Jd "ehime" Daniel

# copy our ini file
COPY ./conf/local.ini /usr/local/etc/php/conf.d/local.ini

# set timezone
RUN rm /etc/localtime && ln -s /usr/share/zoneinfo/America/Los_Angeles /etc/localtime && "date"
...
# might suggest laravel instead
# re: https://laravel.com/docs/5.6/installation
## symfony installer
...
RUN echo  'alias dev="php bin/console   --env=dev"'   >> ~/.bashrc                                \
  && echo 'alias qa="php bin/console    --env=qa"'    >> ~/.bashrc                                \
  && echo 'alias uat="php bin/console   --env=uat"'   >> ~/.bashrc                                \
